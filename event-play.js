...
function saveLocal(){if(!user)return;user.eventTeam={main:[...selectedMain],special:selectedSpecial};user.eventMusic=selectedSong;localStorage.setItem('realyze_user_cache',JSON.stringify(user));return syncUser()}
async function syncUser(){try{const d=db(),s=await d?.auth?.getSession();if(d&&s?.data?.session?.user?.id)await d.from('profiles').update({game_data:user}).eq('id',s.data.session.user.id)}catch(e){console.warn('event user sync',e)}}
...
function startBattle(remoteInfo){
 const mine=remoteInfo.remote?remoteInfo.mine:teamSnapshot();
 const rivalIds=remoteInfo.remote?(remoteInfo.opp?.team||[]).map(x=>typeof x==='string'?x:x.id):(teamSnapshotFromAI().main);
...
function teamSnapshotFromAI(){return {main:['lumina','akito','kohane'],special:'kohane'}}
function buildTeam(ids){return ids.map(id=>CHARS[id]).filter(Boolean).map(c=>({...c,bp:bp(c),cd:[0,0,0]}))}
